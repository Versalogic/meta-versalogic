...
*								  (XR17D158/154/152)
*       ChangeLog:
*	   for    	: LINUX 2.6.32 and newer 
*	   date   	: April 2020
*	   version	: 1.0 
*	 Note: Added VID/PID for Acces I/O P104-COM232-8 card. 
*
*       ChangeLog:
...
serialxr_set_special_baudrate(struct uart_port *port,unsigned int special_baudrate)
{
	...
	if(!((up->deviceid == 0x152)||(up->deviceid == 0x154)||(up->deviceid == 0x158)||(up->deviceid == 0x10a8)))
	{
...
init_one_xrpciserialcard(struct pci_dev *dev, const struct pci_device_id *ent)
{
	...
	if((priv->dev->device == 0x152)	||(priv->dev->device == 0x154)||(priv->dev->device == 0x158)||(priv->dev->device == 0x10a8))
